...
public partial class SolutionHandler : ISolutionHandler
{
    ...
    private static Dictionary<int, IDictionary<int, SolutionMetadata>> GatherPuzzleSolutions()
    {
        ...
        var solutionTypes = AppDomain
            .CurrentDomain.GetAssemblies()
            .SelectMany(x => x.GetTypes())
            .Where(x => typeof(ISolution).IsAssignableFrom(x) && !x.IsAbstract)
            .ToList();
        var years = solutionTypes
            .Select(t =>
                Convert.ToInt32(YearRegex().Match(t.FullName!).Value, CultureInfo.InvariantCulture)
            )
            .Distinct();
        ...
        foreach (var year in years)
        {
            ...
            foreach (
                var solutionType in solutionTypes.Where(t =>
                    t.FullName!.Contains(
            ...
            {
                var puzzleAttribute = solutionType
                    .GetCustomAttributes(typeof(PuzzleAttribute), false)
                    .OfType<PuzzleAttribute>()
                    .FirstOrDefault();
                var day =
                    puzzleAttribute?.Day
                    ?? Convert.ToInt32(
                        NumberRegex().Match(solutionType.Name).Value,
