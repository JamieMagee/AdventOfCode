...
{
    private readonly Regex rulesRegex = new(
        @"(?<name>(\w+\s?)+):( (?<lower>\d+)\-(?<upper>\d+)(?: or)?){2}"
    );
    private readonly Regex ticketRegex = new(@"^((?:\d,?)+)$", RegexOptions.Multiline);

    public Func<string, bool> FieldFilter { get; set; } =
        s => s.StartsWith("departure", StringComparison.Ordinal);

    protected override string Part1(string input)
    {
        var ticketMatches = this.ticketRegex.Matches(input);
        var nearbyTickets = ticketMatches
            .ToArray()[1..]
            .Select(l => l.Value.Split(',').Select(int.Parse));

        var rulesMatches = this.rulesRegex.Matches(input);
        var rules = rulesMatches.Select(m =>
            (
                name: m.Groups["name"].Value,
                validators: m.Groups["lower"]
                    .Captures.EquiZip<Capture, Capture, Func<int, bool>>(
                        m.Groups["upper"].Captures,
                        (l, u) =>
                            n =>
                                n >= int.Parse(l.Value, CultureInfo.InvariantCulture)
                                && n <= int.Parse(u.Value, CultureInfo.InvariantCulture)
                    )
            )
        );
    ...
    public override async Task<string> Part2Async(string input)
    {
        ...
        var nearbyTickets = ticketMatches
            .ToArray()[1..]
            .Select(l => l.Value.Split(',').Select(int.Parse).ToArray());

        var rulesMatches = this.rulesRegex.Matches(input);
        var rules = rulesMatches
            .Select(m =>
                (
                    name: m.Groups["name"].Value,
                    validators: m.Groups["lower"]
        ...
        return map.Where(r => this.FieldFilter(r.Name))
