    ...
    public sealed class Day20 : SolutionBase
    {
        public override Task<string> Part1Async(string input)
        {
            ...
            return Task.FromResult(
                cornerTiles.Aggregate(1L, (acc, tile) => acc * tile.Id).ToString()
            );
        }
        ...
        private static List<Tile> ParseTiles(string input)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                if (lines[i].StartsWith("Tile", StringComparison.Ordinal))
                {
                    ...
                    i++;
                    while (
                        i < lines.Length && !lines[i].StartsWith("Tile", StringComparison.Ordinal)
                    )
                    {
                        grid.Add(lines[i]);
                        i++;
        ...
        private static List<Tile> FindCornerTiles(
            List<Tile> tiles,
            Dictionary<string, List<Tile>> borderMap
        )
        {
        ...
        private Tile[,] AssemblePuzzle(List<Tile> tiles, Dictionary<string, List<Tile>> borderMap)
        {
            ...
            for (int rotation = 0; rotation < 4; rotation++)
            {
                for (int flip = 0; flip < 2; flip++)
                {
                    ...
                    if (
                        borderMap[topBorder].Count == 1
                        && borderMap[topReversed].Count == 1
                        && borderMap[leftBorder].Count == 1
                        && borderMap[leftReversed].Count == 1
                    )
                    {
                        grid[0, 0] = topLeft;
                        used.Add(topLeft.Id);
            ...
            for (int row = 0; row < size; row++)
            {
                for (int col = 0; col < size; col++)
                {
                    ...
                    foreach (var candidate in tiles.Where(t => !used.Contains(t.Id)))
                    {
                        ...
                        var orientedTile = FindCorrectOrientation(
                            candidate,
                            targetBorder,
                            isTopBorder
                        );
                        if (orientedTile != null)
                        {
                            if (row > 0 && col > 0)
                            {
                                ...
                                if (orientedTile.GetBorders()[3] != leftBorder)
                                    continue;
                            }
        ...
        private static Tile? FindCorrectOrientation(
            Tile tile,
            string targetBorder,
            bool isTopBorder
        )
        ...
        private static bool IsSeaMonsterAt(
            string[] image,
            string[] seaMonster,
            int startRow,
            int startCol
